...
### v1.4 機能（実装済み）
- **日記項目**: 記録入力画面のメモ直後に「📓 日記（任意）」複数行テキストエリアを追加（5000 文字以内）。キーワード検索対象に含む。CSV 入出力に統合
- DB スキーマに `diary` カラム追加（既存記録は null）
- CSV を 9 列形式に拡張。v1.0/v1.1 (5 列)、v1.2 (7 列)、v1.3 (8 列) のインポートも引き続き可能（後方互換）

詳細: [docs/design/19-v1.4-diary-field.md](docs/design/19-v1.4-diary-field.md)
...
| [19 v1.4 日記項目](docs/design/19-v1.4-diary-field.md) | 日記 TextArea（複数行、5000 文字、検索対象）の設計 |
