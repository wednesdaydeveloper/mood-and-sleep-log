...
- **year 選択時は下段「睡眠時間帯」を非表示**にする
  （月平均の就寝・起床時刻は分散が大きく、平均値で帯を描いても誤解を招きやすいため。
  気分・睡眠時間の月推移を見るのが year ビューの主目的）
- 各点・各帯タップで該当日の値を表示
...
- **year 選択時は本パネル自体を非表示にする**（v1.2 以降の挙動）
- 集約ロジック自体は残しており、データポップアップには月平均の代表区間が含まれる
- 0件月の集約は intervals が空配列。仮に再表示する場合も列のみ・帯なしで描画される
