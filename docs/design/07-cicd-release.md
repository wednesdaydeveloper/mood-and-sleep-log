...
| プロファイル | 用途 |
|--------------|------|
| development | dev client、ローカル開発 |
| preview | TestFlight / 内部配布、動作確認 |
| production | 自分の本番端末用（自分専用なのでストア公開はしない） |

### コマンド
```bash
eas build --profile preview --platform all
eas update --channel preview --message "M3完了"
```

## 4. OTA 更新（EAS Update）

- JSコードのみの変更は OTA で配信可能（ネイティブモジュール変更時はストア再ビルド）
- `production` チャンネルには手動で確認後にプロモート
