...
export function DataPointPopup({ point, ratio, containerWidth }: DataPointPopupProps) {
  ...
  return (
    <View
      style={[styles.popup, { left, width: POPUP_WIDTH, backgroundColor: colors.popupBg }]}
      pointerEvents="none"
      accessibilityLiveRegion="polite"
    >
      <Text style={[styles.title, { color: colors.popupText }]}>{formatTitle(point.dateIso)}</Text>
      <Text style={[styles.line, { color: colors.popupText }]}>
        気分: {point.mood !== null ? `${moodIcon(point.mood)} ${formatMood(point.mood)}` : '—'}
      </Text>
      <Text style={[styles.line, { color: colors.popupText }]}>
        睡眠: {formatSleep(point.sleepMinutes)}
      </Text>
